import { compareHashes, detectHashCollision, expandShortHash, findHashIndex, removeDuplicateHashes, shortenHash, sortHashes } from '@/core/hash/HashUtils';
import {
    INVALID_HASHES,
    VALID_HASHES,
    expectArgumentError,
...
describe('Hash Utils Functions', () => {
    describe('compareHashes', () => {
        it('should return true for identical hashes', () => {
            expect(compareHashes(VALID_HASHES.sha1_1, VALID_HASHES.sha1_1)).toBe(true);
        });

        it('should return true for case insensitive comparison', () => {
            expect(compareHashes(VALID_HASHES.lowercase, VALID_HASHES.uppercase)).toBe(true);
        });

        it('should return false for different hashes', () => {
            expect(compareHashes(VALID_HASHES.sha1_1, VALID_HASHES.sha1_2)).toBe(false);
        });
    ...
    describe('shortenHash', () => {
        ...
        it('should throw error for length less than 4', () => {
            expectArgumentError(
                () => shortenHash(VALID_HASHES.sha1_1, 3),
                'length'
            );
        });
        ...
        it('should throw error for length greater than 40', () => {
            expectArgumentError(
                () => shortenHash(VALID_HASHES.sha1_1, 41),
                'length'
            );
        });
    });
    ...
    describe('Edge cases and performance', () => {
        ...
        it('should handle empty arrays', () => {
            expect(compareHashes('a1b2c3d4e5f6789012345678901234567890abcd', 'a1b2c3d4e5f6789012345678901234567890abcd')).toBe(true);
        });
