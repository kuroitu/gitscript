import { calculateHash, calculateHashFromBuffer, calculateHashFromMultiple, calculateHashFromObject } from '@/core/hash/HashCalculator';
import { compareHashes, detectHashCollision, expandShortHash, removeDuplicateHashes, shortenHash } from '@/core/hash/HashUtils';
import { isValidHash, verifyHashIntegrity, verifyObjectHashIntegrity } from '@/core/hash/HashValidator';
...
describe('Hash Module Integration', () => {
    ...
    it('should handle error cases gracefully', () => {
        ...
        expect(() => compareHashes(invalidHash, 'valid-hash')).toThrow(InvalidHashError);
        expect(verifyHashIntegrity(invalidHash, 'content')).toBe(false);

        // 無効な入力での計算
        expect(() => calculateHash(123 as any)).toThrow(TypeError);
        expect(() => calculateHashFromBuffer('not-buffer' as any)).toThrow(TypeError);
        expect(() => calculateHashFromMultiple('not-array' as any)).toThrow(TypeError);
    });
});