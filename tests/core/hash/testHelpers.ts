 ...
 * エラーメッセージの変更に柔軟に対応
 */
export function expectInvalidHashError(fn: () => void, hash?: string): void {
  if (hash) {
...
export function generateLargeHashArray(size: number): string[] {
  return Array.from(
    { length: size },
    (_, i) => `a${i.toString().padStart(39, '0')}`,
  );
}
...
export const VALIDATION_TEST_CASES = {
  ...
  invalidHashes: [
    ...
    {
      name: 'invalid char',
      input: INVALID_HASHES.invalidChar,
      expected: false,
    },
