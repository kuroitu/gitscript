import { validateArray, validateBuffer, validateRange, validateString } from '@/core/utils/validators';
import {
...
describe('Validation Functions', () => {
    ...
    describe('validateString', () => {
        it('should return string for valid string values', () => {
            ...
            expect(validateString(TEST_VALUES.emptyString)).toBe(TEST_VALUES.emptyString);
            expect(validateString(TEST_VALUES.unicodeString)).toBe(TEST_VALUES.unicodeString);
            expect(validateString(TEST_VALUES.longString)).toBe(TEST_VALUES.longString);
        });

        it('should throw TypeError for non-string values', () => {
            expectTypeError(() => validateString(TEST_VALUES.number), 'string', 'value');
            expectTypeError(() => validateString(TEST_VALUES.booleanTrue), 'string', 'value');
            expectTypeError(() => validateString(TEST_VALUES.null), 'string', 'value');
            expectTypeError(() => validateString(TEST_VALUES.undefined), 'string', 'value');
            expectTypeError(() => validateString(TEST_VALUES.array), 'string', 'value');
            expectTypeError(() => validateString(TEST_VALUES.object), 'string', 'value');
            expectTypeError(() => validateString(TEST_VALUES.buffer), 'string', 'value');
        });

        it('should use custom parameter name in error message', () => {
            expectTypeError(() => validateString(TEST_VALUES.number, 'customParam'), 'string', 'customParam');
            expectTypeError(() => validateString(TEST_VALUES.booleanTrue, 'myParam'), 'string', 'myParam');
        });
    ...
    describe('validateArray', () => {
        it('should return array for valid array values', () => {
            ...
            expect(validateArray(TEST_VALUES.emptyArray)).toBe(TEST_VALUES.emptyArray);
            expect(validateArray(TEST_VALUES.nestedArray)).toBe(TEST_VALUES.nestedArray);
            expect(validateArray(TEST_VALUES.mixedArray)).toBe(TEST_VALUES.mixedArray);
        });

        it('should throw TypeError for non-array values', () => {
            expectTypeError(() => validateArray(TEST_VALUES.string), 'array', 'value');
            expectTypeError(() => validateArray(TEST_VALUES.number), 'array', 'value');
            expectTypeError(() => validateArray(TEST_VALUES.booleanTrue), 'array', 'value');
            expectTypeError(() => validateArray(TEST_VALUES.null), 'array', 'value');
            expectTypeError(() => validateArray(TEST_VALUES.undefined), 'array', 'value');
            expectTypeError(() => validateArray(TEST_VALUES.object), 'array', 'value');
            expectTypeError(() => validateArray(TEST_VALUES.buffer), 'array', 'value');
        });

        it('should use custom parameter name in error message', () => {
            expectTypeError(() => validateArray(TEST_VALUES.string, 'customParam'), 'array', 'customParam');
            expectTypeError(() => validateArray(TEST_VALUES.number, 'myParam'), 'array', 'myParam');
        });
    ...
    describe('validateBuffer', () => {
        it('should return Buffer for valid Buffer values', () => {
            ...
            expect(validateBuffer(TEST_VALUES.emptyBuffer)).toBe(TEST_VALUES.emptyBuffer);
            expect(validateBuffer(TEST_VALUES.largeBuffer)).toBe(TEST_VALUES.largeBuffer);
        });

        it('should throw TypeError for non-Buffer values', () => {
            expectTypeError(() => validateBuffer(TEST_VALUES.string), 'Buffer', 'value');
            expectTypeError(() => validateBuffer(TEST_VALUES.number), 'Buffer', 'value');
            expectTypeError(() => validateBuffer(TEST_VALUES.booleanTrue), 'Buffer', 'value');
            expectTypeError(() => validateBuffer(TEST_VALUES.null), 'Buffer', 'value');
            expectTypeError(() => validateBuffer(TEST_VALUES.undefined), 'Buffer', 'value');
            expectTypeError(() => validateBuffer(TEST_VALUES.array), 'Buffer', 'value');
            expectTypeError(() => validateBuffer(TEST_VALUES.object), 'Buffer', 'value');
        });

        it('should use custom parameter name in error message', () => {
            expectTypeError(() => validateBuffer(TEST_VALUES.string, 'customParam'), 'Buffer', 'customParam');
            expectTypeError(() => validateBuffer(TEST_VALUES.number, 'myParam'), 'Buffer', 'myParam');
        });
    ...
    describe('validateRange', () => {
        ...
        it('should use custom parameter name in error message', () => {
            expectArgumentError(() => validateRange(-1, 0, 10, 'customParam'), 'customParam');
        ...
        // パラメータ化テスト
        RANGE_VALIDATION_TEST_CASES.forEach(({ name, value, min, max, shouldThrow }) => {
    ...
    describe('Edge cases and special values', () => {
        ...
        it('should handle edge cases for validateArray', () => {
            ...
            expect(validateArray(Array.from('hello'))).toEqual(['h', 'e', 'l', 'l', 'o']);
        });
