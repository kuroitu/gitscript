...
describe('MapDeltaCalculator', () => {
  describe('calculateMapDelta', () => {
    ...
    it('複数のキーが変更された場合の差分を計算する', () => {
      const oldMap = new Map([['a', 1], ['b', 2], ['c', 3]]);
      const newMap = new Map([['a', 4], ['b', 5], ['c', 6]]);
      const result = calculateMapDelta(oldMap, newMap);
    ...
    it('数値キーのMapの差分を計算する', () => {
      const oldMap = new Map([[1, 'one'], [2, 'two'], [3, 'three']]);
      const newMap = new Map([[1, 'one'], [2, 'two'], [3, 'four']]);
      const result = calculateMapDelta(oldMap, newMap);
      expect(result.delta.changeCount).toBeGreaterThan(0);
      // 数値キーは'key_3'形式に変換される可能性がある
      const hasKey3 = result.delta.changes['3'] || result.delta.changes['key_3'];
    ...
    it('オブジェクト値のMapの差分を計算する', () => {
      ...
      const oldMap = new Map([['user1', obj1], ['user2', obj2]]);
      const newMap = new Map([['user1', obj1], ['user2', obj3]]);
      const result = calculateMapDelta(oldMap, newMap);
    ...
    it('大きなMapでも正常に動作する', () => {
      const oldMap = new Map(Array.from({ length: 1000 }, (_, i) => [i, `value${i}`]));
      const newMap = new Map(Array.from({ length: 1000 }, (_, i) => 
      ...
      const hasKey500 = result.delta.changes['500'] || result.delta.changes['key_500'];
    ...
    it('Mapの順序は影響しない', () => {
      const map1 = new Map([['a', 1], ['b', 2], ['c', 3]]);
      const map2 = new Map([['c', 3], ['a', 1], ['b', 2]]); // 順序が異なる
      const result = calculateMapDelta(map1, map2);
      expect(result.delta.changeCount).toBe(0);
    });
    ...
    it('キーと値の両方が変更された場合の差分を計算する', () => {
      const oldMap = new Map([['a', 1], ['b', 2]]);
      const newMap = new Map([['a', 1], ['c', 3]]); // 'b'が削除され、'c'が追加
      const result = calculateMapDelta(oldMap, newMap);
      expect(result.delta.changeCount).toBeGreaterThan(0);
      expect(result.delta.changes['b']).toBeDefined();
